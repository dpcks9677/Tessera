...
| 현재 작업 | `M9-T3` 컨트롤러를 참조 전용으로 전환 (사용자의 베이커·마이그레이터 실행 대기) |
| 다음 행동 | Unity 메뉴에서 Bake Tabletop Prefabs → Migrate Scene To Prefab Instances 실행 후 `M9-T3` 진행 |
| 마지막 완료 작업 | `M9-T2` 런타임 자동 재생성 무력화 (좌표 덮어쓰기·스냅백 제거) |
