...
`M10.5`로 실루엣은 살아났지만 화면이 픽셀아트가 아니라 윤곽선을 그린 3D로 읽혔다. 측정해 보니 테이블 크롭 한 곳의 고유 색이 616색이었다. 픽셀 크기만 3×3으로 키웠을 뿐 24비트 연속 계조가 그대로 남아 있었고, 해상도를 절반으로 낮춰도 612색이 남아 그대로였다. 픽셀아트를 픽셀아트로 만드는 것은 픽셀 크기가 아니라 적은 색 수다.
...
M10.6은 이 기능을 되살리고 세 모드를 런타임에 순환한다. Off, sRGB 채널 단계, 아트 가이드 팔레트다. 팔레트는 `docs/art_style_guide.md` §4 컬러 토큰 8개에서 토큰당 4단계 명암 램프를 만들어 32색을 구성한다. Bayer 4×4 디더는 가상 격자 좌표로 인덱싱해 픽셀 격자와 정합시킨다.

| ID | 작업 | 상태 | 완료 조건 |
|---|---|---|---|
| `M10.6-T0` | 현재 상태 백업 커밋 | `DONE` | M10.5 결과가 커밋으로 보존됨 |
| `M10.6-T1` | sRGB 양자화와 Bayer 디더 | `DONE` | 포스터라이즈 모드에서 화면이 붉게 물들지 않고 계조가 단계로 끊김 |
| `M10.6-T2` | 아트 가이드 토큰 팔레트 | `DONE` | 32색 램프가 생성되고 셰이더가 최근접 색으로 치환함 |
| `M10.6-T3` | 3단 모드 전환 | `DONE` | `Q`와 화면 버튼으로 Off/Steps/Palette를 즉시 순환함 |
| `M10.6-T4` | 엣지 강도 재조정 | `DEFERRED` | `M10.8-T6`으로 이관. 저해상도 실렌더로 바꾸면 임계값 기준 자체가 달라지므로 그때 함께 맞춘다 |
| `M10.6-T5` | 문서화 | `DONE` | 상세 문서에 색 양자화 절 추가, 마일스톤 요약 반영 |
...
## M10.7. 트레이 확대와 보드 좌표 상수 단일화

`M10.6`에서 던지는 주사위를 15% 키우자 트레이가 상대적으로 좁아졌다. 착지 좌표는 `SourceToUnityScale`을 통해 `DieSize`에 비례해 함께 퍼지는데 트레이 배율은 고정 0.05였기 때문이다. 트레이 STL 메시 배율을 0.06으로 올려 여유를 회복한다.
...
| ID | 작업 | 상태 | 완료 조건 |
|---|---|---|---|
| `M10.7-T1` | 트레이 배율 0.05 → 0.06 | `DONE` | 프리팹 스케일과 씬 인스턴스 Y가 새 배율과 일치함 |
| `M10.7-T2` | 트레이 좌표 상수 단일화 | `DONE` | 컨트롤러의 중복 상수 3개 제거, `TrayVisualY`가 배율에서 파생됨 |
| `M10.7-T3` | 플레이 영역 경계 상수 정의 | `DONE` | 소스 단위와 월드 단위 경계가 정의되고 Z 부호 반전 근거가 주석에 남음 |
| `M10.7-T4` | 문서화 | `DONE` | 마일스톤 요약과 실행 기록 반영 |
...
| 후속 | 신설한 플레이 경계 상수는 `M10.9` 프리셋 베이커가 물리벽과 착지 판정에 사용한다 |

## M10.8. 셀 셰이딩 전환과 픽셀 격자 정합

`M10.6`의 포스트 색 양자화는 색 수를 616에서 30까지 줄이고도 화면을 픽셀아트로 만들지 못했다. 가설이 절반만 맞았다. 픽셀아트를 만드는 것은 적은 색 수가 아니라 적은 평면 영역 수다. 포스트 양자화는 합성된 프레임에 걸리므로 색 경계가 밝기 그라데이션의 등치선에 생기고, 그 등치선은 휜 노멀 위에서 동심원 밴드가 되어 주사위가 구르면 표면 위를 흘러간다. 경계가 도형 경계가 아니라 렌더 경계다.
...
문제는 두 좌표계가 `DieSize = 0.78`에서만 맞물렸다는 것이다(`preset-studio/src/YachtTrayModel.js`의 `UNITY_SYNCED_TRAY_SCALE = (1.62 * 0.05) / 0.78`). `M10.6`에서 주사위만 0.897로 키우자 착지 좌표가 트레이 대비 15% 넓게 퍼져 주사위가 림을 뚫었다. `M10.7`의 트레이 확대가 여유를 되찾아 주지만, 두 좌표계가 서로 다른 경로로 스케일된다는 구조 자체는 그대로다.
...
| ID | 작업 | 상태 | 완료 조건 |
|---|---|---|---|
| `M10.9-T1` | EditMode 물리 스텝 확인 | `DONE` | 플레이 모드 없이 프리뷰 씬의 물리 씬을 스텝해 낙하가 재현됨 |
| `M10.9-T2` | 물리벽 리그와 주사위 바디 | `DONE` | 림 경계 박스 5개와 D6·D8 바디가 `DiceBoardMetrics`에서 파생됨 |
| `M10.9-T3` | 시뮬레이터와 좌표 역변환 | `DONE` | 인코딩한 좌표가 재생 변환을 거쳐 원래 자세로 돌아옴 |
| `M10.9-T4` | 평가기와 층화 선별기 | `DONE` | 세 기준이 구현되고 판정 시간이 구간에 균등 배치됨 |
| `M10.9-T5` | 20종 일괄 베이킹 | `DONE` | 20파일 × 20클립이 생성되고 기존 검증 도구가 통과함 |
| `M10.9-T6` | 문서화 | `DONE` | 마일스톤 요약과 실행 기록 반영 |
