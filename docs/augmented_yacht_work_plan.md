...
| 항목 | 현재 값 |
|---|---|
| 전체 상태 | M1~M6 완료, M7 중단 (M7-T1~T5 완료); `M7.5` 증강 시스템 구조 리팩토링 100% 완료 (80/80 테스트 통과) |
| 현재 마일스톤 | `M7.5` 완료 → `M8` 증강 요트 로컬 핫시트 완성 (또는 M7 오디오 폴리싱 복귀) 대기 |
| 현재 작업 | `M7.5` 완료 보고 및 다음 단계 결정 |
| 다음 행동 | `M8-T1` 플레이어 교대 가림 화면 또는 사용자 결정에 따라 `M7` 연출/오디오 잔여 작업 재개 |
| 마지막 완료 작업 | `M7.5-R6` 구 상태 필드 제거 및 States 위임 전환 (80/80 전체 통과) |
| 차단 요소 | 기술적 차단 없음 |
| 마지막 갱신일 | 2026-09-03 |
...
| `M7` | 증강 시스템 그래픽 요소 추가 | `TODO` | `M6` | 카드·특수 주사위·발동 연출·상태 표시 완성 (M7-T1~T5 완료 후 `M7.5`를 위해 중단) |
| `M7.5` | 증강 시스템 구조 리팩토링 | `DOING` | `M6` | 발동 시점 인터페이스, 증강별 핸들러·상태 분리, 증강별 회귀 테스트 |
| `M8` | 증강 요트 로컬 핫시트 완성 | `TODO` | `M7`, `M7.5` | 전체 증강 게임 완주 가능 |
| `M9` | 네트워크 준비 검증 | `TODO` | `M8` | 지연·중복·재동기화 대응 |
| `M10` | EOS/Steam 호스트 온라인 | `TODO` | `M9` | 비레이팅 온라인 매치 |
| `M11` | 원격 권위/레이팅 | `DEFERRED` | 별도 기획 | 필요할 때만 진행 |
...
`M7-T5` 증강 고유 아이콘 이식까지 완료한 시점에서, 사용자 지시에 따라 M7을 중단하고 `M7.5`를 먼저 진행한다(`D-019`). 남은 M7 작업은 `M7.5` 완료 후 재개한다.

## M7.5. 증강 시스템 구조 리팩토링

M7.5는 게임 규칙의 동작을 바꾸지 않고 증강 구현 구조만 재구성하는 마일스톤이다. 목적은 증강 추가 비용을 낮추는 것이며, 판단 기준은 "증강 하나를 추가할 때 새 파일 1개와 카탈로그 1줄만 필요한가"다.
...
| `M7.5-R3` | 강화 중 주사위·상시형 이관 | `DONE` | 주사위 6종과 요트 뱅크·추진력·결투·저금통·랜덤박스가 핸들러로 이동함 |
| `M7.5-R4` | 퀘스트 11개 이관 | `DONE` | `AfterScoreCommit` 250줄이 해체되고 퀘스트별 진행·실패·보상이 각 파일에 모임 |
| `M7.5-R5` | 수동 행동 5개 이관 | `DONE` | `LocalGameAuthority.UseAugmentAction`의 증강 ID 하드코딩이 제거되고 핸들러 위임으로 바뀜 |
| `M7.5-R6` | 구 상태 필드 제거 | `DONE` | `YachtAugmentPlayerState`에 공용 필드만 남고 증강별 필드가 `AugmentStateStore` 위임으로 전환됨 |
...
| ID | 작업 | 상태 | 완료 조건 |
|---|---|---|---|
| `M8-T1` | 플레이어 교대 가림 화면 | `TODO` | 상대의 비공개 선택지를 바로 볼 수 없음 |
| `M8-T2` | 플레이어별 드래프트 UI | `TODO` | 현재 플레이어만 선택 가능함 |
| `M8-T3` | 보유 증강 표시 | `TODO` | 대상 족보와 진행 상태를 확인 가능함 |
| `M8-T4` | 증강 수동 행동 UI | `TODO` | 사용 가능 조건과 남은 횟수가 표시됨 |
| `M8-T5` | 시간 관련 증강과 타이머 | `TODO` | 로컬 규칙 기준으로 일관되게 처리됨 |
| `M8-T6` | 게임 종료 요약 | `TODO` | 점수와 증강 결과를 확인 가능함 |
| `M8-T7` | 반복/무작위 전체 게임 테스트 | `TODO` | 여러 증강 조합으로 완주함 |
...
| ID | 작업 | 상태 | 완료 조건 |
|---|---|---|---|
| `M9-T1` | 명령/결과/상태 직렬화 테스트 | `TODO` | 왕복 직렬화 후 상태가 동일함 |
| `M9-T2` | `CommandId` 중복 방지 | `TODO` | 동일 명령이 한 번만 적용됨 |
| `M9-T3` | `ExpectedRevision` 충돌 처리 | `TODO` | 오래된 명령이 명확한 오류로 거부됨 |
| `M9-T4` | 지연 시뮬레이터 | `TODO` | 200~500ms 지연에서도 UI가 안정적임 |
| `M9-T5` | 순서 뒤바뀜/재전송 테스트 | `TODO` | 잘못된 순서의 결과가 상태를 되돌리지 않음 |
| `M9-T6` | 전체 스냅샷 재동기화 | `TODO` | 임의 시점에서 화면을 복구할 수 있음 |
| `M9-T7` | `HostGameAuthority` 루프백 구현 | `TODO` | 실제 플랫폼 없이 호스트/게스트 흐름 검증 |
...
## M10. EOS/Steam 호스트 온라인

플랫폼은 구현 시작 전에 다시 결정한다. EOS와 Steam을 동시에 결합하지 않고 하나의 최소 수직 흐름부터 완성한다.

| ID | 작업 | 상태 | 완료 조건 |
|---|---|---|---|
| `M10-T1` | 인증/로비 플랫폼 결정 기록 | `TODO` | EOS/Steam 선택 이유와 범위가 기록됨 |
| `M10-T2` | 로그인과 플레이어 ID 매핑 | `TODO` | 재접속 시 동일 플레이어를 식별함 |
| `M10-T3` | 친구 초대 또는 로비 참가 | `TODO` | 2인이 같은 매치에 참가함 |
| `M10-T4` | 호스트 선택 | `TODO` | 양쪽이 동일한 호스트를 인식함 |
| `M10-T5` | 명령 전송과 결과 브로드캐스트 | `TODO` | 게스트 명령이 호스트에서 검증됨 |
| `M10-T6` | 전체 상태 동기화 | `TODO` | 늦게 도착하거나 재접속한 클라이언트 복구 |
| `M10-T7` | 호스트 종료 정책 | `TODO` | 매치 종료 또는 마이그레이션 정책이 동작함 |
| `M10-T8` | 온라인 전체 게임 검증 | `TODO` | 일반/증강 모드 온라인 완주 |
...
| `D-006` | 2026-08-24 | 레이팅과 원격 권위 서버를 현재 범위에서 연기한다. | 초기 개발·운영 비용 절감 | M11은 `DEFERRED` |
...
| `Q-004` | 온라인 1차 플랫폼은 EOS인가 Steam인가? | `M10-T1` | 크로스플랫폼 목표면 EOS 우선 |
| `Q-005` | 호스트 종료 시 즉시 패배/무효/호스트 이전 중 무엇을 적용하는가? | `M10-T7` | 초기에는 매치 무효 종료 |
