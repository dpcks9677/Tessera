...
| `M10.5-T5` | 기존 필터와 A/B 전환 | `BLOCKED` | F3와 `Edge: ON/OFF` 버튼으로 즉시 교체 가능함 |
| `M10.5-T6` | 반투명 오브젝트 무회귀 | `DONE` | 코스믹 큐브·수정구·모래시계 유리가 외곽선을 받지 않음 |
| `M10.5-T7` | 임계값 튜닝 | `TODO` | 두 해상도에서 실루엣과 챔퍼가 읽히고 회전 중 픽셀 크롤이 없음 |
...
| 알려진 문제 | HUD 디버그 버튼 `960 / 640`과 `Edge: ON/OFF`가 눌러도 반응하지 않음. M10.5 이전부터 있던 해상도 버튼도 같이 죽어 있어 원인은 엣지 기능이 아니라 디버그 버튼 클릭 경로로 추정 |
| 남은 작업 | `M10.5-T5` 버튼 클릭 경로 수정, `M10.5-T7` 임계값 실측 튜닝과 픽셀 크롤 확인 |
