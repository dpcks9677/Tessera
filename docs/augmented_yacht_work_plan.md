...
| `M12` | 픽셀 필터 엣지 검출 도입 | `TODO` | `M11` | 뎁스 외곽선·노멀 하이라이트, 기존 필터와 A/B 전환 |
| `M13` | 픽셀 필터 색 양자화 | `TODO` | `M12` | sRGB 포스터라이즈·아트 가이드 팔레트·Bayer 디더, 3단 모드 전환 |
...
M7은 규칙 결과를 변경하지 않는 프레젠테이션 계층에서 카드, 특수 주사위, 점수표 상태, 공통 VFX를 실제 게임 품질로 완성하는 마일스톤이다. 상세 작업 상태와 사용자 협업 기록의 단일 기준은 [`docs/augmented_yacht_m7_graphics_plan.md`](augmented_yacht_m7_graphics_plan.md)에서 관리한다. 에셋 인벤토리, 카드 디자인 수정 사양, 3D 스크롤 리디자인은 그 문서 부록 A~C로 합쳤고 원본은 `docs/archive/`에 있다.
...
M13은 이 기능을 되살리고 세 모드를 런타임에 순환한다. Off, sRGB 채널 단계, 아트 가이드 팔레트다. 팔레트는 `docs/art_style_guide.md` §4 컬러 토큰 8개에서 토큰당 4단계 명암 램프를 만들어 32색을 구성한다. Bayer 4×4 디더는 가상 격자 좌표로 인덱싱해 픽셀 격자와 정합시킨다.
...
| `M16-T10` | 문서화 | `DONE` | `docs/cel_shading_pixel_plan.md` 작성과 마일스톤 요약 반영 |
...
| `M17-T19` | 깃펜 깃털 흰색 전환과 갈라짐 연출 | `DROPPED` | 절차적 깃펜 자체를 폐기하고 외부 로우폴리 모델 `Assets/Art/Reference/quill_pen_low.fbx`로 교체(2026-09-12). 화면이 480x270 격자로 필터링돼 여기서 맞춘 디테일이 남지 않는데도 굽고 눈으로 확인하는 왕복이 계속 필요해 비용이 맞지 않았다. 계획서는 [`docs/quill_feather_appearance_plan.md`](quill_feather_appearance_plan.md)에 폐기 표시와 함께 남김 |
...
- 완료 내용: 계획서 [`docs/quill_hover_writing_animation_plan.md`](quill_hover_writing_animation_plan.md) 작성. 구현은 아직 없다
- 확인한 사실:
