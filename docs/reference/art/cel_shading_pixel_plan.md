...
>
> `M16` 셀 셰이딩의 기술 레퍼런스입니다. **채택은 보류(`DEFERRED`)됐고 인프라만 유지합니다** (`D-037`).

이 문서는 `docs/agent/work_plan.md`의 `M16` 마일스톤을 뒷받침하는 기술 상세다.
...
`D-037`(채택 보류)에서 `m_RenderingMode`는 Forward로 유지하기로 했다. 되돌릴 변경이 아니라 확정된 편차다. `Mobile_Renderer`가 이미 Forward이고 광원이 하나라 Deferred 이점이 없으며, Cel을 재개하면 엣지 피처가 Forward를 요구한다.
...
`Run Pixel Readability Validation` 최종 실행 결과다. 셀 전환은 실제로 걸렸다. 렌더러 337개가 교체됐다.

| 지표 | Baseline | Cel | 판정 |
|---|---|---|---|
| **주사위 면 안쪽 밴드 수** (14x14 패치) | 7 | **5** | 소폭 개선 |
| **주사위 면 안쪽 최대 평면 영역** | 77.04 % | 73.47 % | 사실상 동률 |
| 주사위 바운딩 박스 밴드 수 | 22 | **13** | 개선. 주사위 사이 배경이 셀로 바뀐 몫 |
| 주사위 바운딩 박스 최대 평면 영역 | 4.99 % | 3.59 % | 악화 |
| 전체 프레임 밴드 수 | 43 | 42 | 변화 없음 |
| 전체 프레임 최대 평면 영역 | 3.14 % | **5.51 %** | 개선(+75 %) |
| 굴림 중 최대 변화 셀 비율 | 4.3 % 안팎 | **3.4 % 안팎** | 개선 |

### 10.2 결론: 전제가 틀렸다

가장 중요한 발견은 Cel 쪽 수치가 아니라 **Baseline 쪽 수치**다.

**Baseline에서도 주사위 면 안쪽은 이미 77 % 평면이고 밝기 밴드가 7개뿐이다.** 주사위는 애초에 3D로 읽히는 원인이 아니었다.

§1이 세운 기계론은 "주사위 면의 노멀이 연속적으로 휘어 등치선이 동심원 밴드가 된다"였다. 그 근거는 `DiceMeshFactory`의 구형 노멀이었는데, **그것은 폴백 경로다.** 실제로 쓰이는 것은 `normal_dice.fbx`이고, 그 메시의 면은 이미 거의 평면으로 셰이딩된다. 전제를 확인하지 않고 폴백 코드에서 결론을 끌어낸 것이 잘못이었다.

그래서 노멀 축 스냅(§3.1)은 "핵심"이 아니었다. 고칠 것이 별로 없는 곳을 고쳤다.

남은 밴드 수는 다른 데 있다. 전체 프레임 밴드 수가 Cel에서도 42로 그대로인 것이 그 증거다.

1. **텍스처가 있는 표면.** 나무결 베이스맵은 알베도 자체가 연속으로 변해서 조명을 아무리 계단화해도 밴드가 줄지 않는다. `BuildDiceGraphicsPoC`가 플레이맷 텍스처를 밉맵 없이 Point 필터로 강제하는 것도 네이티브 480에서 잡음을 키운다.
2. **아직 변환하지 않은 특수 연출 셰이더.** `M16-T7` 미착수라 코스믹 큐브·오브의 연속 그라데이션이 그대로다.

### 10.3 그래도 남는 성과

- **픽셀 크롤이 줄었다** (4.3 % → 3.4 % 안팎). 저해상도 실렌더(§5)가 의도대로 동작했다. 이것은 셀 셰이딩과 독립적이라 Cel을 채택하지 않아도 따로 살릴 수 있다.
- **전체 프레임 최대 평면 영역이 75 % 늘었다** (3.14 % → 5.51 %). 화면이 전체적으로는 평평해졌다.
- 측정 과정에서 셀 셰이더의 실제 버그 두 건을 잡았다(§10.4). 채택 여부와 무관하게 고쳐야 할 것들이었다.

### 10.4 측정이 잡아낸 셰이더 버그 두 건

둘 다 지표가 아니었으면 화면만 봐서는 원인을 짚기 어려웠을 것이다.

**1. 주사위가 자기 그림자를 받고 있었다.**

`Renderer.receiveShadows`는 URP Lit 전용 플래그다. 커스텀 라이팅인 셀 셰이더에는 통하지 않는데, `DiceVisualPool`이 그 플래그만 끄고 있었다. 주사위는 그림자를 프록시 큐브(`ShadowProxy`, 크기 `SourceDiceSize` 1.62)로 캐스팅하는데, 셀 셰이더가 그림자를 무조건 샘플링하니 **자기 프록시가 만든 그림자를 자기 얼굴에 받았다.** 하드 `step` 때문에 그 경계가 면 한가운데를 칼같이 갈랐다.

재질 프로퍼티 `_ReceiveShadows`를 추가해 주사위는 끄고 나머지는 켰다.

**2. 직교 카메라인데 원근 방식으로 시선 벡터를 구했다.**

`normalize(GetCameraPositionWS() - positionWS)`는 원근 투영용이다. 직교에서는 시선 벡터가 화면 전체에서 같아야 하는데, 이렇게 구하면 한 면 안에서도 값이 변한다. 하드 림의 `step` 경계가 그 변화를 타고 면을 갈라, 주사위 면 안쪽 최대 평면 영역이 **정확히 50.00 %** 로 반토막 났다. URP의 `GetWorldSpaceNormalizeViewDir`로 교체해 66.84 %, 이어서 아래 측정 버그까지 고친 뒤 73.47 %가 됐다.

### 10.5 측정 도구에서 함께 고친 것

지표 자체의 결함 네 건을 순서대로 잡았다. 하나하나가 잘못된 결론으로 이어질 뻔했다.

| 결함 | 증상 | 수정 |
|---|---|---|
| 화면 전체만 쟀다 | 계획서에는 주사위 크롭이라 적어 놓고 전체 프레임으로 구현 | 주사위 크롭과 면 안쪽 패치 지표 추가 |
| 크롭 축이 월드 X/Y였다 | 카메라가 기울어져 크롭이 129x10셀로 납작 | 카메라 `right`/`up` 축으로 교체 → 129x32 |
| 전환 직후 프레임이 쓰레기값 | 렌더 타깃 재생성 직후를 읽어 Cel 변화율 100.00 % | 워밍업 두 프레임 폐기 |
| 두 캡처가 사각형을 공유 | 주사위 자세가 달라 크기가 1셀 어긋나면 면적이 조용히 0 반환. Baseline이 0.00 %로 나왔다 | 연출 방식별로 사각형 분리 |

지표를 만들 때는 지표 자체를 먼저 의심해야 한다.

### 10.6 채택 판단

**현재 구현은 M16 완료 조건을 충족하지 못한다.** 주사위 밴드 수 목표 3 이하에 못 미치고, 애초에 주사위가 문제가 아니었다.

다만 "Cel을 버린다"가 자동으로 따라오지는 않는다. 결정에 필요한 것은 화면 확인이다. 지표는 평면 영역이 늘었다고 말하는데, 그것이 픽셀아트로 읽히는지는 다른 질문이다.

`M16-T7`(특수 연출 셰이더)과 텍스처 표면 처리가 남아 있어, 현재 화면은 셀 전환의 최종 모습이 아니다.
...
## 12. 매듭 (2026-09-07, `D-037`)

셀 셰이딩 채택을 **보류**한다. 버리지도, 채택하지도 않는다.

### 12.1 판단

M16 완료 조건을 충족하지 못했고(주사위 밴드 목표 3, 실측 5), §10.2가 보였듯 전제 자체가 틀렸다. 주사위는 3D로 읽히던 원인이 아니었다. 남은 밴드는 텍스처 표면과 미변환 특수 연출 셰이더(`M16-T7`)에 있고, 그 둘을 처리해야 채택 여부를 화면으로 판단할 수 있다. 그래픽 폴리시는 `M17` 뒤로 미루기로 했으므로(`work_plan` `D-036`) 지금 판단을 강행하지 않고 인프라만 남긴 채 보류한다.

### 12.2 유지·이월

| 항목 | 처리 |
|---|---|
| `RenderStyle` 토글, `CelSurface`, `CelStyleSwitcher`, `PixelReadabilityMetrics`, 측정 도구 | 유지. 기본값 `Baseline`이라 화면은 M15까지와 같다 |
| `M16-T7` 특수 연출 셰이더 축소 | `DEFERRED`. 그래픽 패스에서 Cel 재개 시 |
| 텍스처 표면 처리, 엣지 임계값 재조정, Cel 육안 비교 | 그래픽 패스로 이월 |
| `PC_Renderer.m_RenderingMode` | Forward(`0`) 유지. §6.4 참조 |
| 저해상도 실렌더, 셀 셰이더 버그 2건 수정 | 이미 트리에 반영. Cel 채택과 독립적으로 유효 |

채택이 확정되면 별도 정리 마일스톤에서 Baseline 경로를 제거한다(`work_plan` `D-034`).
