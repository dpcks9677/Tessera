...
>
> `M12` 엣지 검출 셰이더의 기술 레퍼런스입니다. 구현은 완료됐습니다.

이 문서는 `docs/agent/work_plan.md`의 `M12` 마일스톤을 뒷받침하는 기술 상세다.
마일스톤 문서에는 배경과 태스크만 두고, 셰이더 공식·이식 함정·튜닝 기록은 여기에 남긴다.

## 1. 배경 — 기존 픽셀 필터가 못 하던 것

M12 이전의 픽셀 필터는 픽셀화만 했다. 월드 카메라가 `1920×1080` 렌더 타깃에 그리면
전체 화면 `RawImage`에 붙은 `DicePoC/PixelUpscale`이 UV를 `640×360`(또는 `480×270`)
...
- 명암만으로 형태를 읽히게 하려고 개별 에셋의 색값을 손으로 조정해야 했다
  (`docs/agent/m7_graphics_spec.md` 부록 B 참조).
...
원본은 여기에 블룸과 최종 업스케일 패스를 더하지만, M12는 두 엣지 지표만 가져온다.
...
| `F1` / `F2` | 내부 해상도 `640×360` / `480×270`. 시작값은 `480×270` |
...
엣지 검출을 넣어도 화면이 픽셀아트가 아니라 **윤곽선을 그린 3D 모델링**으로 읽혔다.
윤곽만 또렷해지고 면 내부 계조는 그대로라 오히려 그 인상이 강해졌다.

측정으로 확인했다. 테이블 크롭(498,800픽셀)의 고유 색이 **616색**이다. 픽셀 크기만 3×3으로
키웠을 뿐 24비트 연속 계조가 그대로 남아 있었다. 해상도를 절반(`320×180`)으로 낮춰도 612색이
남아 여전히 모자이크 처리한 3D였다. **픽셀아트를 픽셀아트로 만드는 것은 픽셀 크기가 아니라
적은 색 수다.**

### 9.2 기존 `_Quantize`가 화면을 붉게 만든 이유

양자화 코드는 원래부터 있었지만 `ApplyRenderSettings()`가 매번 `0`으로 강제해 죽어 있었다.
켜면 화면이 시뻘겋게 변했기 때문인데, 기능 문제가 아니라 **색공간 버그**였다.

프로젝트는 Linear 색공간(`ProjectSettings.asset: m_ActiveColorSpace: 1`)이다. 셰이더가 받는 값은
선형인데 그걸 그대로 6단계로 잘랐다. 선형 `1/6 = 0.167`은 sRGB로 약 `0.45`다.
어두운 웜톤 씬(R 선형 `~0.09`, G `~0.02`, B `~0.015`)에서 **R만 `0.167`로 올라가고 G·B는 `0`으로
떨어져** 순수 채도 빨강이 된다.

그래서 양자화는 반드시 sRGB 공간에서 한다.

```
srgb  = LinearToSRGB(color.rgb)
srgb += Bayer 4×4 오프셋
srgb  = 양자화(채널 단계 또는 팔레트 최근접)
color = SRGBToLinear(srgb)
```

`LinearToSRGB` / `SRGBToLinear`는 URP `Core.hlsl`이 가져오지 않는다.
`com.unity.render-pipelines.core/ShaderLibrary/Color.hlsl`을 명시적으로 포함해야 한다.

### 9.3 세 모드

`DicePixelUpscale.shader`의 `_Quantize`를 모드로 쓴다. 프로퍼티 이름과 기본값 `0`은 유지했으므로
씬에 구워진 재질을 건드릴 필요가 없었다.

| 모드 | 값 | 동작 |
|---|---|---|
| Off | `0` | 격자 스냅만. 양자화 이전과 같은 화면 |
| Steps | `1` | sRGB 채널을 `_ColorSteps`(기본 6)단계로 포스터라이즈 |
| Palette | `2` | 아트 가이드 팔레트에서 sRGB 거리 최근접 색으로 치환 |

`Q` 키와 화면 좌상단 `Quant:` 버튼이 세 모드를 순환한다. 기본값은 Off다.

### 9.4 팔레트 구성

`TesseraPixelPalette`가 `docs/reference/art_style_guide.md` §4의 컬러 토큰 8개를 단일 출처로 삼아
토큰당 명도 배율 `0.35 / 0.65 / 1.0 / 1.45`의 4단계 램프를 만든다. 총 32색이다.
아트 방향이 바뀌어 토큰을 고치면 화면 팔레트도 따라간다.

명도가 `1`을 넘으면 그냥 자르지 않고 초과분만큼 채도를 낮춰 하이라이트로 뺀다.
그렇게 하지 않으면 이미 명도가 `1`인 `color-light-warm-key`의 위쪽 두 칸이 같은 색이 되어
팔레트 칸을 낭비한다.

셰이더에는 `float4 _PaletteColors[32]` 유니폼 배열로 넘긴다. 32회 루프의 최근접 탐색은
텍스처 페치 없이 ALU만 쓰므로 3D LUT 없이 충분하다.

### 9.5 디더링

Bayer 4×4 임계 행렬을 **가상 격자 좌표**(`floor(uv * _VirtualResolution)`)로 인덱싱한다.
화면 픽셀 좌표를 쓰면 디더 무늬가 픽셀 격자보다 촘촘해져 3×3 블록 안에서 무늬가 깨진다.
강도는 `_DitherStrength`(기본 `0.5`)이며 `0`이면 완전히 꺼진다.

### 9.6 검증 결과 (2026-09-05)

같은 크롭(`120,120`~`700,980`)의 고유 색 수다.

| 모드 | 고유색 | 관찰 |
|---|---|---|
| Off | 616 | 기준. 연속 계조 |
| Steps | **55** | 붉은 물듦 재발 없음. 원본 웜톤 색상을 유지한 채 계조만 끊김. 디더 무늬가 레트로 질감을 더함 |
...
아트 가이드 토큰 8개에는 **중성색과 시안 계열이 없다.** 그래서 팔레트 모드에서 다음이 흡수된다.

- 흰 주사위 → 베이지·브라운
- 코스믹 큐브의 시안 네온 → 인디고 블루
- 석재 룬 원반 → 브라운

전체 세피아 통일감은 오히려 강해지지만 주사위·석재·발광체의 구분이 약해진다.
보강하려면 중성 그레이 램프와 시안 토큰을 팔레트에 추가하고
`TesseraPixelPalette.MaxColors`와 셰이더 배열 크기를 함께 늘려야 한다.
이번 범위에서는 하지 않았다.

## 10. 남은 확인

- 주사위를 실제로 굴리는 동안 외곽선이 반짝이지(pixel crawl) 않는지 확인.
  `docs/archive/plans/poc_engine_decision.md`(원래 `Assets/Docs/Decision.md`)에 열린 리스크로 적혀 있다. 정지 화면 검증만 마쳤다.
