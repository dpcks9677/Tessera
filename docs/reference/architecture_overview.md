...
- **주요 게임 1 (현재 구현)**: **증강 요트 다이스 (Augmented Yacht Dice)**
  - 고전 요트 다이스(Yacht Dice)의 족보 규칙에 **45종의 고유 특수 능력(증강 / Augments)** 카드를 결합한 2인 대전 전략 보드게임 (현재 로컬 2인 핫시트 지원).
...
| **재굴림** | `RollsRemaining` (`YachtGameCore`) | 턴마다 기본 2회 제공되는 추가 주사위 굴림 기회. 남은 횟수를 셉니다. |
...
| **드래프트** | `Draft` / `DraftRound` | 1·6·9 라운드에 플레이어가 3장의 증강 카드 중 하나를 선택하는 단계. 판정은 `YachtAugmentRuntime.IsDraftRound`. |
...
│   └── AugmentedYacht/     # 증강 요트 다이스 전용 시스템
│       ├── Logic/          # 증강 런타임, 점수 엔진, 45종 증강 핸들러 (Tessera.Games.Yacht)
│       └── Presentation/   # 점수판, UI 뷰, 턴 연출, 카메라/조명 리그, 컨트롤러
│                           #   (Tessera.Games.AugmentedYacht)
└── Network/                # 빈 폴더. 온라인 통신 계층 자리만 잡아 둠 (파일 0개)
```
...
- [RollCosmicCube.cs](../../Assets/Scripts/Tabletop/RollCosmicCube.cs): 주사위 굴리기 버튼 역할을 겸하는 성운 큐브. 앞서 같은 역할을 하던 `RollOrb`는 폐기됐습니다.
...
└── Dice Graphics PoC                       ← 씬의 단일 루트. AugmentedYachtController(합성 루트)가 여기 붙습니다
    ├── Graphics Layout                     ← 3D 월드. 조명·카메라·테이블 프롭이 전부 이 아래
    │   ├── Key Light                       (주광)
    │   ├── Fill Light                      (보조광)
    │   ├── Full Field World Camera         (3D 월드를 렌더 타깃에 그립니다)
    │   │   └── Crisp UI Camera             (픽셀 필터를 거치지 않는 선명한 숫자 전용)
    │   ├── Display 1 Camera
    │   ├── 3D Wood Planks Table            [프리팹]
    │   ├── 3D Fabric Runner                [프리팹]
    │   ├── Yacht Tray Visual               [프리팹]
    │   ├── 3D Stone Augment Card Tray      [프리팹]
    │   ├── 3D Layered Parchment Score Sheet[프리팹]
    │   ├── 3D Inkwell and Quill Decoration [프리팹]
    │   ├── 3D Parchment Paperweight        [프리팹]
    │   ├── 3D Roll Cosmic Cube             [프리팹]
    │   ├── 3D Reroll Counter Bar           [프리팹]
    │   ├── 3D Hourglass Timer              [프리팹]
    │   ├── 3D Cozy Beeswax Candle Decoration [프리팹]
    │   ├── 3D Runic Slate & Crystal Matrix [프리팹]
    │   ├── 3D Trinket Cluster (Ring, Brooch, Crystal) [프리팹]
    │   └── 3D Turn Balance Indicator       [프리팹]
    └── Pixel Presentation                  ← Screen Space Overlay 캔버스 (sortingOrder 100)
        ├── Game Area
        │   ├── Point Upscale               (월드 렌더 타깃을 점 샘플링으로 확대하는 RawImage)
        │   └── Crisp UI Overlay
        ├── Status
        └── Debug · KeyLightToggle · RuneFxDebug · RuneStoneDebug  (디버그 버튼)
```

> **프롭은 전부 `Graphics Layout`의 직계 자식인 프리팹 인스턴스입니다.** `TableRoot`나 `Props` 같은
> 중간 그룹 노드는 없습니다. 프리팹 원본은 `Assets/Prefabs/Tabletop/`에 있고,
> `Tessera/Tabletop/Bake Tabletop Prefabs` 메뉴가 씬의 프롭을 프리팹으로 다시 굽습니다.
>
> 테이블 상판의 나무 판자와 천 러너만은 프리팹에 구워져 있지 않고
> [TabletopSurfaceBuilder.cs](../../Assets/Scripts/Games/AugmentedYacht/Presentation/TabletopSurfaceBuilder.cs)가
> 게임 시작 시 절차적으로 만듭니다.
...
sequenceDiagram
    ...
    Unity->>AYC: Start()
    AYC->>LGA: ExecuteAsync(YachtGameCommand { Type = StartGame }) 발행
...
flowchart LR
    ...
    subgraph Command ["2. 명령 (Command)"]
        Router -->|생성| Cmd["YachtGameCommand<br/>Type = CommitScore<br/>(Category, ExpectedRevision)"]
    end
...
2. **명령 캡슐화**: [ParchmentScoreSheet.cs](../../Assets/Scripts/Games/AugmentedYacht/Presentation/ParchmentScoreSheet.cs)가 점수 변수를 직접 수정하지 않고, [YachtInputRouter.cs](../../Assets/Scripts/Games/AugmentedYacht/Presentation/YachtInputRouter.cs)를 통해 `YachtGameCommand`(`Type = YachtCommandType.CommitScore`)를 생성합니다. 명령은 타입별 클래스가 아니라 `YachtCommandType` 열거형으로 구분되는 단일 클래스입니다.
...
### 6.2 45종 증강(Augments) 확장 시스템 (OCP 준수)
...
> 📖 **45종 증강 전수 목록, 8단계 턴 훅 오케스트레이션 및 점수 계산 파이프라인**에 대한 상세 기술 문서는 [`augments_specification_and_status.md`](augments_specification_and_status.md)를 참고하세요.
...
classDiagram
    ...
    class IDiceBonusProvider {
        <<interface>>
        +int CalculateDiceBonus(AugmentQueryContext, IReadOnlyList~YachtDieState~)
    }

    class IDiceLayoutProvider {
        <<interface>>
        +int RequiredDiceSlots
        +void ConfigureDice(AugmentDiceContext)
    }

    class IAfterScoreCommit {
        <<interface>>
        +void AfterScoreCommit(AugmentCommitContext)
    }
    ...
    IAugmentHandler <|.. EnhanceAugment
    IAugmentHandler <|.. QuestAugment
    EnhanceAugment <|-- GoldenDie
    IDiceLayoutProvider <|.. GoldenDie
    IDiceBonusProvider <|.. GoldenDie
    QuestAugment <|-- BountyHunter
    IAfterScoreCommit <|.. BountyHunter
```

> `IAugmentHandler` 자체에는 `Apply()` 같은 만능 진입점이 **없습니다.** 계약은 `Id`, `Order`,
> `CreateDefinition()` 셋뿐이고, 실제 발동 시점은 핸들러가 추가로 구현한 훅 인터페이스가 정합니다.
> 디스패처가 구현된 인터페이스를 보고 호출 대상을 고릅니다.
>
> 구현체는 보통 `ModificationAugment` / `EnhanceAugment` / `QuestAugment` 추상 기반을 상속해
> `DisplayName`과 `Description`만 채웁니다. 클래스 이름에 `Handler` 접미사는 붙이지 않습니다.
...
| **코스믹 큐브 (Roll Cosmic Cube)** | 굴리기 버튼 클릭 시 회전 및 성운 빛 방출 | [RollCosmicCube.cs](../../Assets/Scripts/Tabletop/RollCosmicCube.cs) |
...
1. **핸들러 파일 생성**
   - 위치: `Assets/Scripts/Games/AugmentedYacht/Logic/Augments/{Modification|Enhance|Quest}/`
   - 파일명은 증강 이름 그대로입니다. `Handler` 접미사를 붙이지 않습니다 (`GoldenDie.cs`, `BountyHunter.cs`).
   - `Core/`는 인프라(인터페이스·디스패처·카탈로그·상태 저장소) 전용입니다. 핸들러를 넣지 않습니다.
2. **카테고리 기반 클래스를 상속**하고 필요한 훅 인터페이스만 추가로 구현합니다.
   ```csharp
   // 실제 예: Assets/Scripts/Games/AugmentedYacht/Logic/Augments/Enhance/GoldenDie.cs 와 같은 형태
   using System.Collections.Generic;

   namespace Tessera.Games.Yacht   // Logic/ 아래는 전부 이 네임스페이스입니다
   {
       /// <summary>눈이 7이면 +7점 보너스를 얻습니다.</summary>
       public sealed class LuckySeven : EnhanceAugment, IDiceBonusProvider
       {
           public override string Id => YachtAugmentRuntime.LuckySevenId;

           public override string DisplayName => "행운의 7";

           public override string Description => "눈이 7이면 +7점 보너스를 얻습니다.";
   ...
   ```
   `CreateDefinition()`은 기반 클래스가 이미 구현했습니다. `EnhanceAugment`가 `Kind`를
   `YachtAugmentKind.Enhance`로 채워 넣으므로 다시 쓸 필요가 없습니다. 별도의 희귀도(rarity) 개념은
   이 프로젝트에 없습니다.
3. **ID 상수 추가**: `YachtAugmentRuntime`의 ID 상수 목록에 `public const string LuckySevenId = "lucky-seven";`을 넣습니다.
...
### 11.1 코딩 규약 핵심 ([coding_conventions.md](coding_conventions.md))
- **언어**: 코드 주석, 커밋 메시지, 문서는 **한국어**로 씁니다. 다만 **식별자는 영문으로만** 짓습니다. 클래스·메서드·필드·지역 변수는 물론 테스트 메서드명까지 해당합니다.
...
- **마일스톤 작업 계획서**:
  - [work_plan.md](../agent/work_plan.md)와 [solid_refactoring_work_plan.md](../agent/solid_refactoring_work_plan.md)에서 현재 진행 중인 태스크(`DOING`)와 다음 목표(`TODO`)를 추적합니다. 결정 기록은 [decisions.md](../agent/decisions.md), 과거 세션 기록은 [session_log.md](../agent/session_log.md)에 있습니다.
