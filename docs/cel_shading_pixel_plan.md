...
| 항목 | 결과 |
|---|---|
| 컴파일 | 신규·수정 스크립트 전부 오류 0 |
| 셰이더 | `CelSurfaceTests`의 `shader.isSupported` 단언 통과. `Tessera/CelSurface`가 실제로 컴파일되고 이 플랫폼에서 지원됨. 세 패스의 상수 버퍼 정합도 여기서 함께 걸러짐 |
| Tessera EditMode | **121/121 통과** (기준선 107 + 신규 14: `CelSurfaceTests` 7, `PixelReadabilityMetricsTests` 6, 씬 재질 계약 1) |
| 전체 EditMode | 809개 중 801 통과, 3 실패, 5 건너뜀. 실패 3건은 모두 `UnitySkills` 패키지 자체 테스트로 이 저장소와 무관 |
| 화면 확인 | 육안 비교는 아직 하지 않았다. 아래 지표만 확보했다 |

### 10.1 지표 측정 결과 (480x270, quantize Off)

`Run Pixel Readability Validation` 실행 결과다. 셀 전환은 실제로 걸렸다. 렌더러 337개가 교체됐다.

| 지표 | Baseline | Cel | 판정 |
|---|---|---|---|
| **주사위 밴드 수** | 22 | **14** | 줄었으나 완료 조건 3(금속 4) 이하에 한참 못 미침 |
| **주사위 최대 평면 영역** | 4.99 % | **3.95 %** | **악화**. 평면이 더 좁아졌다 |
| 전체 프레임 밴드 수 | 43 | 42 | 사실상 변화 없음 |
| 전체 프레임 최대 평면 영역 | 3.14 % | 4.62 % | 개선 |
| 굴림 중 최대 변화 셀 비율 | 4.32 % | **3.38 %** | 개선. 저해상도 실렌더가 효과를 냈다 |

**결론: 현재 구현은 M16 완료 조건을 충족하지 못한다.** 채택할 수 없다.

명확히 개선된 것은 픽셀 크롤 하나다. 저해상도 실렌더(§5)가 의도대로 동작해 셰이딩·텍스처 디테일의 지글거림을 줄였다.

핵심 목표였던 "적은 평면 영역"은 실패했다. 밴드 수가 22에서 14로 준 것은 조명 응답이 계단화되긴 했다는 뜻이지만, 목표치 3의 다섯 배다. 게다가 주사위 크롭의 최대 평면 영역이 오히려 줄었다. 화면이 더 평평해진 것이 아니라 더 잘게 쪼개졌다는 뜻이다.

### 10.2 원인 후보

확인하지 않은 가설이다. 다음 작업의 출발점으로 적어 둔다.

1. **크롭에 배경이 많이 섞였다.** 129x32 크롭은 주사위 다섯 개를 감싸는 바운딩 박스라 사이 간격과 위아래 테이블이 함께 들어온다. 주사위 한 개만, 그것도 몸체 안쪽만 재야 셰이딩 자체를 볼 수 있다. 지금 지표는 "주사위 주변 영역"을 재고 있다.
2. **테이블 텍스처가 네이티브 480에서 앨리어싱한다.** `BuildDiceGraphicsPoC`가 플레이맷 텍스처를 밉맵 없이 Point 필터로 강제한다. 1920으로 그린 뒤 점 샘플링할 때는 화면 공간에서 매끄러웠지만, 480으로 직접 그리면 픽셀마다 나뭇결의 다른 지점을 찍어 잡음이 된다. 전체 프레임 밴드 수가 42로 그대로인 것과 주사위 크롭 평면 영역이 줄어든 것이 이 가설과 맞는다.
3. **특수 연출 셰이더가 그대로다.** `M16-T7`이 미착수라 코스믹 큐브·오브의 연속 그라데이션이 화면에 남아 있다.
4. **셀 셰이더 자체가 덜 계단화됐을 수 있다.** 앰비언트 상수항이 밴드 사이를 메우거나, 하드 림이 밴드를 추가하고 있을 수 있다.

1번과 2번이 지표를 오염시켰을 가능성이 크므로, 4번을 의심하기 전에 **주사위 한 면만 잘라 재는 것**이 먼저다.

### 10.3 측정 도구에서 함께 고친 것

측정하면서 도구 자체의 결함 두 건을 찾아 고쳤다.

- **크롭 축이 틀렸다.** 월드 X/Y 오프셋으로 주사위 사각형을 만들었는데, 이 카메라는 테이블을 내려다보게 기울어져 있어 월드 Y가 화면에서 크게 눌린다. 크롭이 129x10셀로 납작하게 나왔다. 카메라의 `right`/`up` 축으로 바꿔 129x32가 됐다.
- **전환 직후 프레임이 쓰레기값이었다.** 연출 방식을 바꾸면 렌더 타깃을 다시 만드는데, 그 직후 캡처가 아직 그려지지 않은 내용을 읽어 Cel 변화율이 100.00%로 나왔다. 워밍업 두 프레임을 버리도록 고쳤다.

두 결함 모두 첫 측정에서 잘못된 결론을 낼 뻔했다. 지표를 만들 때는 지표 자체를 먼저 의심해야 한다.
